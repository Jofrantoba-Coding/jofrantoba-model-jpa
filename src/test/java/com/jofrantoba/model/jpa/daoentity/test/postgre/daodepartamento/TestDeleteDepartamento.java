 ...
 */
@Log4j2
public class TestDeleteDepartamento extends TestAbstract {

    @Test
    void deleteDepartamentoById() throws Exception {
        ...
        try {
            Departamento d = dao.findById(1L);
            if (d != null) {
                dao.delete(d);
                log.info("Departamento eliminado: {} - {}", d.getCodigoDepartamento(), d.getDescripcion());
            } else {
                log.warn("Departamento no encontrado para eliminar");
            }
            tx.commit();
        } catch (Exception e) {
            log.error("Error al eliminar departamento", e);
            if (tx.isActive()) {
                tx.rollback();
            }
        }
    ...
    @Test
    void deleteMultiplesDepartamentos() throws Exception {
        InterDaoDepartamento dao = buildDao();
        SessionFactory sesionFactory = PSF.getInstance().getPSF("postgre");
        Transaction tx = sesionFactory.getCurrentSession().beginTransaction();
        try {
            ...
            dao.deleteByIds(ids);
            tx.commit();
            log.info("Departamentos eliminados: {}", ids);
        } catch (Exception e) {
            log.error("Error al eliminar múltiples departamentos", e);
            if (tx.isActive()) {
                tx.rollback();
            }
        }
    ...
    @Test
    void deleteDepartamentosInactivos() throws Exception {
        InterDaoDepartamento dao = buildDao();
        SessionFactory sesionFactory = PSF.getInstance().getPSF("postgre");
        Transaction tx = sesionFactory.getCurrentSession().beginTransaction();
        try {
            ...
            tx.commit();
            log.info("Se eliminaron {} departamentos inactivos", eliminados);
        } catch (Exception e) {
            log.error("Error al eliminar departamentos inactivos", e);
            if (tx.isActive()) {
                tx.rollback();
            }
        }
    ...
    @Test
    void truncateDepartamentos() throws Exception {
        InterDaoDepartamento dao = buildDao();
        SessionFactory sesionFactory = PSF.getInstance().getPSF("postgre");
        Transaction tx = sesionFactory.getCurrentSession().beginTransaction();
        try {
            ...
            tx.commit();
            log.info("Tabla departamentos truncada. Se eliminaron {} registros", eliminados);
        } catch (Exception e) {
            log.error("Error al truncar departamentos", e);
            if (tx.isActive()) {
                tx.rollback();
            }
        }

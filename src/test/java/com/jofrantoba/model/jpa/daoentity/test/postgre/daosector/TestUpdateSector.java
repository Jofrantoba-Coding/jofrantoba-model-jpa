 ...
 */
@Log4j2
public class TestUpdateSector extends TestAbstract {
    ...
    void updateDescripcion() throws Exception {
        ...
        try {
            dao.updateDescripcion(1L, "SECTOR 01 MODIFICADO");
            tx.commit();
            log.info("Descripcion actualizada en sector 1");
        } catch (Exception e) {
    ...
    @Test
    void marcarSectorInactivo() throws Exception {
        InterDaoSector dao = buildDao();
        SessionFactory sesionFactory = PSF.getInstance().getPSF("postgre");
        Transaction tx = sesionFactory.getCurrentSession().beginTransaction();
        try {
            ...
            tx.commit();
            log.info("Sector 2 marcado como inactivo");
        } catch (Exception e) {
            log.error("Error al marcar sector inactivo", e);
            if (tx.isActive()) {
                tx.rollback();
            }
        }

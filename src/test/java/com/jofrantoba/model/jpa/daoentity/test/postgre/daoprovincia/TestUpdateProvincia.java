 ...
 */
@Log4j2
public class TestUpdateProvincia extends TestAbstract {
    ...
    void updateDescripcion() throws Exception {
        ...
        try {
            dao.updateDescripcion(1L, "LIMA PROVINCIA");
            tx.commit();
            log.info("Descripcion actualizada en provincia 1");
        } catch (Exception e) {
    ...
    @Test
    void marcarProvinciaInactiva() throws Exception {
        InterDaoProvincia dao = buildDao();
        SessionFactory sesionFactory = PSF.getInstance().getPSF("postgre");
        Transaction tx = sesionFactory.getCurrentSession().beginTransaction();
        try {
            ...
            tx.commit();
            log.info("Provincia 2 marcada como inactiva");
        } catch (Exception e) {
            log.error("Error al marcar provincia inactiva", e);
            if (tx.isActive()) {
                tx.rollback();
            }
        }
